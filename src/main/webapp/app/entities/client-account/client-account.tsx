  ...
  return (
    <div>
      <h2 id="client-account-heading" data-cy="ClientAccountHeading">
        <Translate contentKey="qrMenuApp.clientAccount.home.title">Client Accounts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="qrMenuApp.clientAccount.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/client-account/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            ...
            &nbsp;
            <Translate contentKey="qrMenuApp.clientAccount.home.createLabel">Create new Client Account</Translate>
          </Link>
        </div>
      </h2>
        ...
        {clientAccountList && clientAccountList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="qrMenuApp.clientAccount.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('fullName')}>
                  <Translate contentKey="qrMenuApp.clientAccount.fullName">Full Name</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('mainAddress')}>
                  <Translate contentKey="qrMenuApp.clientAccount.mainAddress">Main Address</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('logoUrl')}>
                  <Translate contentKey="qrMenuApp.clientAccount.logoUrl">Logo Url</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('isDeleted')}>
                  <Translate contentKey="qrMenuApp.clientAccount.isDeleted">Is Deleted</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {clientAccountList.map((clientAccount, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{clientAccount.fullName}</td>
                  <td>{clientAccount.mainAddress}</td>
                  <td>{clientAccount.logoUrl}</td>
                  <td>{clientAccount.isDeleted ? 'true' : 'false'}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="qrMenuApp.clientAccount.home.notFound">No Client Accounts found</Translate>
            </div>
          )
        )}
      </div>
